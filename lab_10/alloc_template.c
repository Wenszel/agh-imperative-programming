...
int read_char_lines(char *array[]) {
    ...
    while (fgets(buff, BUF_SIZE, stdin)){
        char *line = (char *) malloc(((strlen(buff) + 1)* sizeof(char)));
        strcpy(line, &buff[0]);
        array[i] = line;
        i++;
        buff[0] = '\0';
...
int read_int_lines(line_type lines_array[]) {
    ...
    while (fgets(buff, BUF_SIZE, stdin)){
        ...
        line_type line = {&numbers[start], amount, average};
...
int cmp (const void *a, const void *b) {
    ...
    if (line1->average > line2->average) {
        return 1;
    } else if (line1->average < line2->average) {
        return -1;
    } else {
        return 0;
    }
